...
# Remove all but the first of each group of consecutive targets; this makes it easier to
# spot other optimisations.
...
def remove_nop_branches(bytecode_function):
    ...
    for i, instruction in enumerate(bytecode_function.ops):
        ...
        if not (instruction.is_a('BRNCH') and next_instruction and next_instruction.is_target() and
                instruction.operands[0] == next_instruction.operands[0]):
            new_ops.append(instruction)
        else:
            changed = True
    bytecode_function.ops = new_ops
...
def replace_branch_to_exit(bytecode_function):
    changed = False
    targets = build_target_dictionary(
            bytecode_function, lambda instruction: instruction.is_a('LEAVE', 'RET'))
    for instruction in bytecode_function.ops:
...
# If we have any kind of branch whose target is a BRNCH, replace the first branch's target
# with the BRNCH's target (i.e. just branch directly to the final destination in the first
# branch).
def snap_branch(bytecode_function):
    changed = False
    targets = build_target_dictionary(
            bytecode_function, lambda instruction: instruction.is_a('BRNCH'))
...
def remove_dead_code(bytecode_function):
    ...
    bytecode_function.ops = list(
            itertools.chain.from_iterable(itertools.compress(blocks, block_reachable)))
    return not all(block_reachable)


# A CASEBLOCK instruction is followed by the 'otherwise' instructions. If the 'otherwise'
# instructions end with a terminator, the CASEBLOCK+otherwise instructions form an
# isolated block which can be moved around freely. If such a block is preceded by a BRNCH,
# we move it to the end of the function - this may allow the BRNCH to be optimised away.
# (It would be correct and mostly harmless to move *any* isolated CASEBLOCK+otherwise
# instruction block to the end of the function, but it would introduce unnecessary
...
def move_case_blocks(bytecode_function):
    ...
    for i, block in enumerate(blocks):
        if (block_target[i] and len(block) > 1 and block[1].opcode == CASE_BLOCK_OPCODE and
            block[-1].is_terminator() and blocks[i-1][-1].is_a('BRNCH')):
            tail.extend(block)
        else:
...
def block_deduplicate(bytecode_function):
    blocks, block_target, block_isolated = (
        get_target_terminator_blocks_with_isolated_flag(bytecode_function))

    # Compare each pair of non-anonymous blocks (ignoring the initial target instruction);
    # if two are identical and one of them is never entered by falling through from the
    # previous block, we can delete that one and replace all references to its target with
    # the target of the other block.
    alias = {}
    unwanted = set()
...
def block_move(bytecode_function):
    # In order to avoid gratuitously moving chunks of code around (which makes it harder
    # to verify the transformations performed by this function are valid), we remove any
...
def peephole_optimise(bytecode_function):
    changed = False
    i = 0
    # add dummy NOPs so we can use ops[i+2] freely
    bytecode_function.ops += [NopInstruction(), NopInstruction()]
    ...
    while i < len(bytecode_function.ops)-2:
        ...
        elif (instruction.is_paired_conditional_branch() and next_instruction.is_a('BRNCH') and
              next_next_instruction.is_target() and
              instruction.operands[0] == next_next_instruction.operands[0]):
        ...
        elif (instruction.is_simple_store() and not instruction.has_side_effects() and
              next_instruction.is_pure_simple_load() and not next_instruction.has_side_effects() and
              instruction.operands[0] == next_instruction.operands[0] and
              instruction.data_size() == next_instruction.data_size()):
            bytecode_function.ops[i] = Instruction(
                    dup_for_store[instruction.opcode], instruction.operands)
            bytecode_function.ops[i+1] = NopInstruction()
            changed = True
        # LLW [n]:SLW [m]:LLW [n] -> LLW [n]:DLW [m] and variations
        elif (instruction.is_pure_simple_load() and instruction == next_next_instruction and
              next_instruction.is_simple_store() and not instruction.has_side_effects() and
              not next_instruction.has_side_effects() and
              not partial_overlap(instruction, next_instruction)):
            bytecode_function.ops[i+1] = Instruction(
                    dup_for_store[next_instruction.opcode], next_instruction.operands)
...
def optimise_load_store(bytecode_function, straightline_ops):
    ...
    # unobserved_stores is a bidirectional dictionary "memory address" <-> "store
    # instruction index"; it allows us to model the access to memory by straightline_ops
    # to see if any stores are provably redundant. Note that when 'del
    # unobserved_stores[address]' removes the last address associated with an instruction
    # index, the instruction index is removed from the inverse dictionary. On the other
    # hand, if 'unobserved_stores[address] = i' replaces the last address associated with
    # an instruction index, an empty list remains in the inverse dictionary. This allows
    # us to distinguish the two cases; the former case indicates a store instruction has
    # had at least one of its effects observed, the latter indicates a store instruction
    # had no observable effect and can be removed.
    ...
    for i, instruction in enumerate(straightline_ops):
        ...
        if is_store: # stores and duplicate-stores
            ...
            # We have to assume this may observe the value of anything except a frame
            # offset which hasn't been exposed via LLA.
            ...
            # We're exiting the current function, so any unobserved stores to frame
            # offsets will never be observed. We model this by treating this instruction
            # as doing a store to each such offset. (LLA doesn't matter here; once this
            # instruction executes any pointer obtained by LLA points to deallocated
            # memory.)
            for address in unobserved_stores.keys():
    ...
    return [op for op in straightline_ops
            if not op.instruction_class == InstructionClass.NOP], changed
...
def get_straightline_blocks(bytecode_function):
    ...
    for i, instruction in enumerate(bytecode_function.ops):
        if (i == 0 or
            is_branch_or_target(instruction) != is_branch_or_target(bytecode_function.ops[i-1])):
            splitter.start_before(i, not is_branch_or_target(instruction))
    return splitter.get_blocks_and_metadata()


# TODO: This is only ever called to do one optimisation at the moment; could I use it
# more?
    ...
    for block, is_straightline in zip(blocks, is_straightline_block):
        if is_straightline:
            for optimisation in optimisations:
                # optimisation function may modify block in place if it wishes, but it may
                # also be more convenient for it to create a new list so we take a return
                # value; it can of course just 'return block' if it does everything in
                # place.
...
# we could set an upper bound, but we don't have that information. (This assumes that it's
# not valid to rely on the order in which the compiler allocates frame objects and access
# the "preceding" object using a negative offer on the LLA result; I don't think this is
...
# forgetting the implicit branch to the target if control falls through the target from
# the immediately preceding instruction), and there are no conditional branches to the
# target, the instruction can be moved immediately after the target.
def tail_move(bytecode_function):
    # Dictionary mapping from a target to the possible instruction which can be moved
    # after it.
    candidates = {}
    ...
    for i, instruction in enumerate(bytecode_function.ops):
        ...
        if instruction.is_a('BRNCH') or instruction.is_target():
            if previous_instruction.is_terminator():
                # Nothing to do:
                # - If 'instruction' is BRNCH, it can never be executed (and has probably
                #   already been optimised away) so we don't need to take it into account.
                # - If 'instruction' is a target, we never fall through it and so don't
                #   need to take its immediate predecessor into account.
                pass
            else:
                # 'previous_instruction' can execute immediately before 'instruction', so
                # record it as a possible candidate for moving. If we've already got a
                # different candidate, we know this target can't be modified.
                target = instruction.operands[0]
                if candidates.setdefault(target, previous_instruction) != previous_instruction:
                    unmodifiable_targets.add(target)
        else:
            # If a target is used in any other instruction, that is a conditional branch
            # of some kind and we know the target cannot be modified.
...
# Used to test for an unlikely case when optimising things like "LLW [n]:SLW [m]:LLW [n]"
# to "LLW [n]:DLW [m]". If m=n this optimisation is valid, but if m=n+1 the final LLW [n]
# is loading a different value than the first and the optimisation cannot be performed.
...
# automatically, get rid of it; this isn't a huge win but it saves a couple of bytes on
...
def optimise(module):
    ...
    # present) and any exported symbols and recursively adding their dependencies. If the
    # data/asm blob is present but unused (unlikely) we will (correctly) remove it, but
    # the main reason for doing this is to remove unused bytecode functions. We do this
    # after optimising the code to take advantage of any dead code removal.
    dependencies = set()
    if len(module.bytecode_functions) > 0 and module.bytecode_functions[-1].is_init():
    ...
    # dependencies now contains only objects which are needed. We preserve the order of
    # things in the input module; this automatically ensure that the data/asm blob comes
    # first and _INIT comes last, and it also avoids gratuitous reordering which makes
    # comparing the input and output difficult.
...
# TODO: Perhaps not worth it, and this is a space-not-speed optimisation, but if it's
# common to CALL a function FOO and then immediately do a DROP afterwards (across all code
# in the module, not just one function), it may be a space-saving win to generate a
# function FOO-PRIME which does "(no ENTER):CALL FOO:DROP:RET" and replace CALL FOO:DROP
# with CALL FOO-PRIME. We could potentially generalise this (we couldn't do it over
# multiple passes) to recognising the longest common sequence of operations occurring
# after all CALLs to FOO and factoring them all into FOO-PRIME.

# TODO: It would seem a good idea to do an initial pass expanding DUP where the preceding
# instruction is something "simple" like LLW [n], because my simplistic optimisation code
# will tend to be thrown by the presence of a DUP. We could then do a final DUPification
# pass (or perhaps include this in the optimisation loop, but only once we run out of
# other changes to make) to re-introduce DUP in cases like the ones we expanded. I haven't
# done this yet because some experiments and manual examination of the self-hosted
# compiler suggest that this would have no benefit for it, and it is my main test case at
# the moment. TODO: On a similar theme, perhaps we should initially expand
# DROP2->DROP:DROP and do a "final" DROP:DROP->DROP2 to reverse this.

# TODO: The peephole optimiser can do things like "LLW [n]:SLW [m]:LLW [n] -> LLW [n]:DLW
# [m]", but we could also do things like "LLW [n]:DLW [m]:SLW [o]:LLW [n] -> LLW [m]:DLW
# [m]:DLW [o]". An arbitrary number of dup-stores after the first LLW would be acceptable,
# not just one; the current peephole optimiser will not do anything if there are any
# dup-stores after the first load instruction.
