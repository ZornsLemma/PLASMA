...
class Module(object):
    ...
    def compact_int_symbol(i):
        ...
        # see the comment in stodci(). For the first character we also exclude A-Z and
        # _ because all normal symbols generated by the PLASMA compiler will start
        # with one of these, so this guarantees our symbols are unique. SFTODO: BE GOOD TO VERIFY THOSE *ARE* THE RIGHT RESTRICTIONS COMPARED TO THE REAL PLASMA COMPILER
        character_set = [chr(x) for x in itertools.chain(range(33, ord('A')), range(ord('z')+2, 127)) if chr(x) not in ['_']]
        if i == 0:
