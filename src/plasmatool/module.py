...
class Module(object):
    ...
    # This function is partly a translation of module-loading code from cmd.pla; comparing
    # it with that code is likely to be helpful when trying to understand it in future.
    @classmethod
    def load(cls, f):
        seg_size = read_u16(f)
        magic = read_u16(f)
        ...
        for i, (rld_type, rld_word, rld_byte) in enumerate(rld):
            if rld_type == 0x02: # code table fixup
                ...
                star_addr = blob.read_u16(addr)
                # TODO: cmd.pla just checks rld_type & 0x10, but let's be paranoid and
                # check for precise values for now.
                if rld_type == 0x91: # external fixup
                    ...
                    for esd_name, esd_flag, esd_index in esd:
                        # TODO: As above, cmd.pla just checks esd_flag & 0x10 but we're
                        # precise.
    ...
    def compact_int_symbol(i):
        ...
        assert i >= 0
        # The VM uppercases symbols as they are inserted into the symbol table, so we must
        # not use both upper and lower case characters in these generated symbols; we
        # exclude lower case a-z since symbols are supposed to be all upper case. We also
        # exclude 'z'+1 because it triggers a corner case in cmd.pla; see the comment in
        # stodci(). For the first character we also exclude A-Z and _ because all normal
        # symbols generated by the PLASMA compiler will start with one of these, so this
        # guarantees our symbols are unique.
        range1 = itertools.chain(range(33, ord('A')), range(ord('z')+2, 127))
        range2 = itertools.chain(range(33, ord('a')), range(ord('z')+2, 127))
        character_set = [chr(x) for x in range1 if chr(x) not in ['_']]
        if i == 0:
        ...
        while i > 0:
            ...
            i = i // base
            character_set = [chr(x) for x in range2]
        return result
    ...
    def split(self, second_module_name):
        ...
        #   newly created name exported by callee_module if they weren't previously
        #   exported.
        callee_module_new_exports = caller_module.labels_referenced().intersection(
                callee_module.bytecode_function_labels())
        ...
        caller_module.esd.entry_dict = {k:v for k,v in caller_module.esd.entry_dict.items()
                                        if v is not None}
...
class RLD(object):
    ...
    def dump(self, outfile, esd):
        ...
        # The first part of the RLD must be what cmd.pla calls the "DeFinition
        # Dictionary".
        ...
        # addresses together we should get an improvement (perhaps a negligible one) in
        # the time taken to load the module.
        pending = []
