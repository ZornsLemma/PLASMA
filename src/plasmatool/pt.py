...
parser = argparse.ArgumentParser(description=
        'PLASMA module tool; disassembles and optimises compiled PLASMA modules.')
...
parser.add_argument('-2', '--second-module-name', dest='name',
        help="name for second module when splitting (defaults to basename of OUTPUT2)")
parser.add_argument('input', metavar='INPUT', type=argparse.FileType('rb'),
        help="input file (compiled PLASMA module)")
parser.add_argument('output', metavar='OUTPUT', nargs='?', type=argparse.FileType('w'),
        default=sys.stdout, help="output file (ACME source file), defaults to standard output")
parser.add_argument('output2', metavar='OUTPUT2', nargs='?', type=argparse.FileType('w'),
        default=None, help="second output file (ACME source file) for module splitting")
