...
int emit_seq(t_opseq *seq)
{
    ...
    // If NO_COMBINE is specified, we immediately emit the pending sequence;
    // this effectively means we only peephole optimise a single statement at a
    // time. This is mainly useful a) for debugging b) as it means the
    // correspondence between source code comments and generated bytecode in the
    // output is much cleaner.
    if (outflags & NO_COMBINE)
