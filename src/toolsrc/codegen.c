...
{
    if (localsize)
        pending_seq = gen_leave(pending_seq, localsize);
    else
        pending_seq = gen_ret(pending_seq);
    emit_pending_seq();
}
void emit_leave_internal(int localsize)
{
    ...
    while (op && (opnext = op->nextop))
    {
        switch (op->code)
        {
            ...
            case LAW_CODE:
                ...
                else if ((pass > 1) && (freeops == 0) && (op->type || !is_hardware_address(op->offsz)))
                    crunched = crunched || try_dupify(op);
                break; // LAW_CODE
            case LOGIC_NOT_CODE:
                switch (opnext->code)
