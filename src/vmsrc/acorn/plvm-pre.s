...
!IFDEF PLAS128 {
	...
	; SFTODO: I HAVE TODOS AROUND RELATING TO THE ANNOYANCE OF MAKING INTERP/IINTER/JITIINTERP ADDRESSES AVAILABLE TO CMD.PLA, BEEN LOOKING HOW APPLE 2 DOES IT - COULD I GET ANY MILEAGE FROM DOING SOME $F4 MANIPULATION VIA PAGE 4 CODE WHICH VECTORS INTO THE VM CODE IN THIS FILE? NOWADAYS WITH THE CHUNKED SYMBOL TABLE PAGE 4 RAM IS NOT SIGNIFICANTLY (A BIT; CHUNKED SYMBOL TABLES CAUSE MINOR "CAN'T FREE INIT" MEMORY WASTES) MORE VALUABLE THAN MAIN RAM, SO MOVING CODE FROM HERE TO PAGE 4 IS NOT AN ENORMOUS DEAL - I THINK THERE'S NO VALUE IN DOING ANY RAM BANK PAGING VIA PAGE 4 CODE, SO ACTUALLY INDIRECTING VIA CODE IN PAGE 4 FOR THE 2/3 ENTRY POINTS IS NOT WORTH IT, BUT ALLOCATING 2x2 or 3x2 BYTES IN PAGE 4 FOR THE ADDRESSES IS PROBABLY THE WAY TO GO FOR ACCESSING THE ADDRESSES - THE VM WOULD INIT THEM ONCE ON STARTUP, AND PLASMA CODE CAN THEN ACCESS THEM VIA A SIMPLE *iinterpptr OR WHATEVER, RATHER THAN NEEDING THE ASM STUB FNS I CURRENTLY HAVE
	LDA	RAMBANK
	STA	$F4
	STA	$FE30
}
...
RET
+	
	; SFTODO: START HACK TO SEE IF HELPS JIT ON PLAS128
	LDA	RAMBANK
	STA	$F4
	STA	$FE30
	; SFTODO: END HACK
...
A1CMD	
; SFTODO: COULD MY PLAS128 JIT PROBLEM BE THAT CALLS FROM JITTED MACHINE CODE INTO BYTECODE WILL SWITCH AWAY FROM BANK 4 AND NOT SWITCH BACK? THE *VM* INTERPRETED CALL/ICAL RESTORE $F4/$FE30 AFTER THE CALL, BUT THE JITTED ONE WILL NOT.
